package comm.savagelook.android;
...
public class UrlJsonAsyncTask extends AsyncTask<String, Void, JSONObject> {
	...
	protected Boolean willExecuteJson;
	
	public UrlJsonAsyncTask(Context context, String errorMessage) {
		this.errorMessage = errorMessage;
		UrlJsonAsyncTask.context = context;
	...
	protected void onPreExecute() {
		willExecuteJson = Util
				.hasInternetAndValidToken( UrlJsonAsyncTask.context );
		if (!this.messageLoading.equals( Util.NO_MESSAGE )) {
			if (willExecuteJson) {
				progressDialog = new ProgressDialog( UrlJsonAsyncTask.context );
				progressDialog.setTitle( this.loadingTitle );
				progressDialog.setMessage( this.messageLoading );
				...
				if (Util.hasValidToken()) {
					Toast.makeText( context, this.errorMessage,
